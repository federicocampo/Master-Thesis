...
%This may be due to the loss of some information linked to the site throughout all the process of extracting a correlation coefficient starting from a time-frequency analysis of two timeseries.
%Accordind to this hypothesis, if we lost site information, the particular trend of a feature we see for example in figure \ref{fig:wavelet_features_raw} is just a casual trend and taking into account all of the features, there is not a specific bias connected to a site.
...
%This result may be the consequence of the little amount of data to make statistical analysis on. Controls and ASD data follow different distribution, both biased by site-related information. These information affect the two distribution in the same manner. If this hypothesis is correct, having little statistics does not allow a proper estimation of the site-related parameters. This results in a slightly biased estimation of parameters
...
%These information are then used in an adversarial way during backpropagation by updating only parameters related to the feature extractor branch in an opposite direction, in order to maximize the loss, as shown in equations \ref{eq:adversarial_updating_parameters}.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{shap/waterfall_plot}
