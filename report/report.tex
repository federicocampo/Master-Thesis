...
Given an input data $\mathbf{x_i}$  and an activation function (sigmoid for example, which we recall is of the form $\sigma(z) = \frac{1}{1+e^{-z}}$) the probability of $x_i$ to belong to the class $y_i = 1$, or in other words, the probability to observ $y_i$ given an input $x_i$ is given by
...
We obtain then
\begin{equation}
log(L) = \sum_{i=1}^m y_i log(\sigma(x_i^T \mathbf{w})) + (1-y_i)log(1-\sigma(x_i^T \mathbf{w}))
\end{equation}

which taken with a negative sign corresponds to the cross-entropy function.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.6\linewidth]{abide/controlasd_per_subdataset}
...
An example to visualize this trend is showed in figure \ref{fig:} and thus, for a better readability we prefer reporting the results in terms of absolute shapley value, where we just consider the module of the shapley values, regardless its positive or negative contribution to the output,
...
\end{subfigure}
\begin{subfigure}[b]{.45\textwidth}
   \includegraphics[width=1\linewidth]{shap/shap_bar_adv}
   \caption{adversarial}
   \label{}
\end{subfigure}
\caption{}
...
We can see from this bar plots of the first 20 most important features that some feature seem to be persistent through all the four pipelines; if we limit our analysis on these first twenty we find that: feature 1521, 777, 3042, 2015 and 592 appear, even if in different order of importance, in all our four analysis; features 3401, 710, 5505 appear in 3 out of 4 analysis, and features 2181, 244, 1507, 2126, 1631, 724 and 1511, appear just in two of them.
Let's see what they represent:
\begin{itemize}
\item Feature 1521: correlation between right middle temporal gyrus and left angular gyrus.
...
\item Feature 3042: correlation between Right frontal temporal cortex and Left middle temporal gyrus
\item Feature 2015: correlation between Left frontal medial cortex and right frontal medial cortex
\item Feature 592: correlation between Right middle temporal gyrus and Left superior temporal gyrus
\item Feature 3401: correlation between Right parahippocampal gyrus (anterior division) and Right parahippocampal gyrus (posterior division)
\item Feature 710: correlation between Right middle temporal gyrus and Right talamus
\item Feature 5505: correlation between Left inferior temporal gyrus (temporooccipital part) and Left planum temporale
\item Feature 2181: correlation between Right middle temporal gyrus (posterior division) and Right subcallosal cortex
\item Feature 244: correlation between Right inferior frontal gyrus (part triangolaris) and Right accombus
\item Feature 1507: correlation between Left angular gyrus and Right accumbus
\end{itemize}
%scrivere anche le altre 3???????
