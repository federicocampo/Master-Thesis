...
%modificatoo
There are different wavelets that satisfy properties \ref{eq:waveletproperties}, such as Haar (fig \ref{fig:wavelet_haar}), Meyer (fig \ref{fig:wavelet_meyer}), Mexican hat (fig \ref{fig:wavelet_mexicanhat})
...
To assess the level of significance of a Wavelet coherence matrix over the noise, the statistical significance level of our values has to be estimated.
...
Our goal is to get a correlation-like coefficient from each two timeseries, as we obtained from Pearson correlation analysis.
We can use wavelet coherence matrix computed for each pair of timeseries, using its coefficients and their phase information to compute the time of in-phase (or in phase opposition) coherence which can be seen as the percentage of time synchronicity (or antisynchronicity) between the two timeseries. \cite{bernas-2018}
...
\label{}
\end{subfigure}
\caption{Correlation matrix created with Wavelet coefficients of in-phase and off-phase percentage. These come from patient 51056, ABIDE I }
\label{fig:win_wout}
\end{figure}
...
\item Putting all the predictions together is possible to assign a class label by majority vote.
\end{enumerate}
...
ComBat technique is based on the simpler Location and scale adjustment methods, but is an improved version of it: it uses the empirical Bayes to improve the estimation of the site parameters: it aims to reduce inter-site variability while preserving biological variability such as differences due to sex, age..
