...
\chapter{Explainable AI: a game theory aproach}
...
and the difference between the number of proton in these two states determine the net magnetization of a body under an external magnetic field.
...
To kick this vector away its equilibrium state, a second external magnetic field is employed under the form of a radiofrequency impulse, with a frequence resonating with the Larmor frequence of the precessing.

A common way to apply this radiofrequency impulse is with the purpose of flipping the magnetization vector and hence the spins in a direction usually perpendicular to the original $B_0$ one, which we can suppose to be along $\hat z$ axis.

We create this way a magnetization component along the x-y plane in the direction of the field applied by the radiofrequency pulse, called transverse magnetization.
After this radio frequency pulse, protons will start rotating in the x-y plane,
%still continuing preceding around their symmetry axis.
but doing so, they gradually lose their initial energy and tend to realign along z-axis.
...
The evolution of the longitudinal and transverse magnitudes are modeled with the introduction of two time constant $T_1 \text{ and } T_2$ respectively.

The process through which the longitudinal magnetization regaing its previous magnitude is called longitudinal relaxation and it occurs because the system goes from an higher energy state (when it is flipped on the x-y plane) to a state of thermodynamic equilibrium with its surroadings.

This process follows an exponential decay in time showh in equation \ref{eq:spinlattice}, with a time constant $T_1$, which for the physical reason underlying it, is also called spin-lattice relaxation time.

\begin{equation}\label{eq:spinlattice}
M_z(t) = M_0 (1-e^{-t/T_1})
\end{equation}

%This relaxation occurs because ocalled longitudinal relaxation , or spin-lattice relaxation, referring to the loss of thermal energy through interaxtion with surroading lattice. The total magnetization along z-axis during time, after the rf impulse, will then regrow along z-axis following:

The transverse magnetization vector follows a slightly different evolution. Before introducing the T2 parameter associated with its evolution, we establish a notation to indicate this vector: since it lies on the x-y plane it has two component so we can write $M_\perp = M_x \hat x + M_y \hat y$.
When studying the evolution of transferse magnetization an other effects has to be taken into account, namely the spin-spin interaction between proton.
If we consider a physical system where proton are immersed in a magnetic field, each proton interacts with this external field plus all the small fields created by the surroading proton and their associated spins. This leads to the presence of a local field for each proton, slightly different from the external magnetic field $B_0$.
These different local fields affects the evolution of the transverse magnetization because they cause a relative dephasing of protons between each other.
According to this, if immediately after the rf-impulse all the spins can be imagined as aligned to the x axis, in time they start to fan out. This effect reduces the total transverse magnetization on the x-y plane.
If we study this evolution from the rotating reference frame, the module of the transverse magnetization follows an exponential decay in the form
\begin{equation}
...
\item The first radio-frequency pulse is applied with an angle of $\pi/2$ in relation to the direction of the external magnetic field $\overrightarrow{B_0}$. This process as mentioned in section \ref{sec:mri_introduction} causes the spin to flip over the transversal plane x-y.
...
\item At the instant t = $\tau $ the second rf impulse is sent. It is created with an angle of $\pi$ compared to the first impulse, so if the first was along the $\hat x$ direction, this point towards $-\hat x$. This turns all spins with a phase $\phi$ to a phase $\pi - \phi$.
...
Between the application of the $\pi$ pulse and the echo time, the acquisition sampling starts.
...
In order to relate a signal with a spatial position, in addition to the initial static magnetic field $B_0$ we have to place a second field which causes a controlled local modification of the magnetic field $B_0$. This additional field needs to be non-uniform and have a lower magnitude of $B_0$ for each point.
...
To acquire frequency information from an image we need to introduce two kind of new gradients (one for each direction): a readout (or frequency encoding gradient) and a phase encoding gradient.

The frequency encoding gradient acts on one direction in the transversal plane; let's identify this direction as the x axis without losing generality. Just like the selection gradient it consists of a linear changing magnetic field to modify the Larmor frequency of the spins along the x axis.
...
Each spatial line of k-space is acquired after a time interval called Repetition time (TR).
.
...
Changes in acquisition sequences and rf pulses allow us to emphasise one of the three fundamental parameters $T_1, T_2, \rho$ where $\rho$ is the proton concentration inside the tissue.
...
At this point in an spin echo sequence, the $\pi$ impulse would be applied; in its place, in GE sequences, the gradient is reversed.
...
As thanks to whis it is susceptible to any chemical variation such as that due to hemoglobin shift.
...
EPI refers to a technique to acquire all the 2D k-space after a single rf impulse. This is achieved by applying short phase encoging gradient blips between spin echoes in a multi echo acquisition.
Before we discuss this method we focus our attention on principal factors that play an important role in this kind of acquisition:
...
as shown in figure \ref{fig:epi-gradients} after the initial RF pulse, together with the gradient for slice selection (G-Select), the acquisition of k-space information starts with the combined action of both the readout and the phase encoding gradients.
From the left bottom as in figure \ref{fig:epi-kspace} the first line is acquired by using the readout gradient. Before acquiring the second line a blip (weas triangular gradient) on the phase encoding gradient occurs. Then the readout gradient is reversed and a second echo signal occurs.



each line along k-frequency  is acquired. it is controlled by the readout gradient (G-Readout).
